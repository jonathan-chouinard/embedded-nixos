# BeagleBoard.org TI-patched Linux Kernel for BeagleBone Black
#
# This packages the TI-patched kernel which includes:
# - Full device tree support for AM335x boards (BBB, BBG, PocketBeagle, etc.)
# - PRU (Programmable Real-time Unit) support
# - TI-specific driver patches and optimizations
# - Cape/overlay support
#
# Based on kernel version 6.6.58 from BeagleBoard.org
# Branch: v6.6.58-ti-arm32-r12
#
# Uses linuxManualConfig for full control over kernel config,
# bypassing NixOS's common-config.nix which adds options not suitable for embedded ARM.
{
  buildPackages,
  linuxManualConfig,
  fetchFromGitHub,
  ...
  version = "6.6.58";
  # modDirVersion must match what the kernel source reports (without TI branch suffix)
  ...
  # This creates a proper .config from BeagleBoard.org's bb.org_defconfig
  # Uses buildPackages (native) for host tools since this runs on the build machine
