# NixOS eMMC installation script for BeagleBone Black
# Copies the running system from SD card to internal eMMC using rsync.
#
# Device layout on BeagleBone Black:
# - SD card: /dev/mmcblk0
# - eMMC:    /dev/mmcblk1
...
check_devices() {
    if [[ ! -b "$SDCARD" ]]; then
        echo "Error: SD card $SDCARD not found"
        exit 1
    fi
    ...
    if [[ "$root_dev" == "${EMMC}"* ]]; then
        ...
        echo "Boot from SD card to install to eMMC"
        exit 1
    fi
}

confirm() {
    echo "Install NixOS from $SDCARD to $EMMC"
    echo "WARNING: This will ERASE ALL DATA on $EMMC"
    echo ""
    echo "Current root device: $(findmnt -n -o SOURCE /)"
    echo ""
...
copy_partition_table() {
    ...
    sfdisk --dump "$SDCARD" | sed '$ d;/last-lba/d;' | sfdisk "$EMMC"
    local last_start
    last_start=$(sfdisk --dump "$SDCARD" | tail -1 | sed -n 's/.*start=\s*\([0-9]*\).*/\1/p')
...
sync_boot() {
    ...
    mount -o ro "${SDCARD}p1" /tmp/emmc-src
...
main() {
    ...
    echo "  1. Power off the BeagleBone"
    echo "  2. Remove the SD card"
    echo "  3. Power on - it will boot from eMMC"
    echo ""
    read -r -p "Power off now? (yes/no): " response
    [[ "$response" == "yes" ]] && poweroff
}
